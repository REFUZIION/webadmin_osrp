...
if (!isset($_SESSION['loggedin'])) {
 ...
 $sql = "SELECT * FROM `log_bans` WHERE 'description' LIKE '%$val%' ORDER BY 'date'";
 if ($result = $conn->query($sql)) {
   ...
   while ($row = $result->fetch_assoc()) {
     ...
     $str .= "<div class='listitem' style='border:1px solid black; margin:3px; padding:2px;'>" . $row['date'] . " " . $row['uid'] . " " . $row['description'] . "</div>";
   }
   $str .= ""; // Leeg gemaakt zodat de geanimeerde achtergrond werkt.
   $result->free();
 }
...
<body class="loggedin">
    ...
    <div class="content">
        ...
        <label value="hoi" for="characters">Search ban logs for:</label>
